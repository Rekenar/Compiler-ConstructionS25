...
public class Parser {
    ...
    private static final String[] name = { // token names for error messages
            ...
            "program", "read", "return", "void", "while", "eof", "case", "default", "switch"
    };
    ...
    private static void Statement() {
        ...
        if (sym == ident) {
            ...
            scan();
            if (breakLab != null) Code.jump(breakLab);
            ...
            scan(); // consume 'switch'
            check(lpar);
            // Evaluate the controlling expression once, store into a temp local
            Operand switchExpr = Expr();
            if (switchExpr.type != Tab.intType)
                error("switch expression must be of type int");
            // We need a fresh local variable to hold the switch value:
            // Tab.insert returns a new Obj of kind Var, with next free local slot.
            Obj tempSwitch = Tab.insert(Obj.Var, "#switch_tmp", Tab.intType);
            // Store the switch expression into that local:
            Code.load(switchExpr);
            Operand tempOp = new Operand(Operand.Local, tempSwitch.adr, Tab.intType);
            Code.assignTo(tempOp);

            check(rpar);
            check(lbrace);

            // We will generate a sequence of comparisons: for each case, compare tmp==CONST?
            // If equal, jump to case‐body. Otherwise fall through to next case.
            // We also keep track of (1) a label to jump to when we see a `break;`, and (2) the default label.
            Label endSwitch = new Label();  // where all 'break;' inside this switch should jump
            Label defaultLabel = null;      // will point to default‐body (if present)
            List<Label> caseLabels = new ArrayList<>();    // labels for each case‐block
            List<Integer> caseValues = new ArrayList<>();  // the integer constants for each case

            // First pass: scan all case clauses, record their constants and allocate labels.
            // Then after we know how many cases, we come back, emit comparisons  jumps for each case.
            // But since our parser is single‐pass, we do it as we go:
            while (sym == case_) {
                scan(); // consume 'case'
                if (sym != number) {
                    error("constant expected in case-label");
                    // recover: assign some dummy value
                }
                int cval = t.numVal;
                scan(); // consume number
                check(semicolon); // Actually grammar uses ":", but in MicroJava scanner, colon is semicolon (;) ?
                // WAIT: MicroJava uses semicolon for statement terminator, not colon.
                // But Java‐style switch uses colon, so we must confirm: in MJ, a 'case' label is written `case 5: ...`
                // So token for ':' is missing. Actually, in the token definitions, ':' is not listed.
                // In MJ grammar, they use ';' for many separators—**but** there is no token for colon (':').
                // We need to treat ':' as a separate token.
                // → We’ll correct: in Scanner, ':' is not recognized. We must add colon to Scanner, Parser.
                // For now, assume that “case 5 :” is scanned as `case` `number` `semicolon` (with colon remapped).
                // So we simply use `check(semicolon)` to match “:” after a case.
                // (Alternatively, if you have added a separate colon token, replace check(semicolon) with check(colon).)

                // Create a new label for this case‐body
                Label caseLab = new Label();
                caseLabels.add(caseLab);
                caseValues.add(cval);
            }

            // Optionally: is there a default label?
            if (sym == default_) {
                scan(); // consume 'default'
                check(semicolon); // match ':'
                defaultLabel = new Label();
            }

            // Now we have a list of all case‐values and their labels; we must compare
            // the switch expression against each value in sequence.
            // For each i in 0..caseLabels.size()-1:
            //    load switchTemp, const caseValues[i], compare, if eq jump to caseLabels[i]
            //
            for (int i = 0; i < caseLabels.size(); i++) {
                // load temp local
                Code.load(tempOp);
                // push the constant
                int val = caseValues.get(i);
                if (0 <= val && val <= 5) Code.put(Code.const0 + val);
                else if (val == -1) Code.put(Code.const_m1);
                else {
                ...
                Code.put(Code.eq);
                // jump if true
                Code.tJump(Code.eq, caseLabels.get(i));
            }
            // If none matched, jump to default (if any) or endSwitch
            ...
            // ───────────────────────────────────────────────────────────────
            // Now actually parse and emit each case’s statements in textual order.
            //
            int caseIndex = 0;
            while (caseIndex < caseLabels.size()) {
                // Position the label for caseIndex
                caseLabels.get(caseIndex).here();
                // Parse all statements until next 'case', 'default', or '}'.
                while (!(sym == case_ || sym == default_ || sym == rbrace)) {
                    Statement();
                }
                caseIndex++;
            }

            // If there was a 'default', position its label and parse its statements.
            if (defaultLabel != null) {
                defaultLabel.here();
                while (sym != rbrace) {
                    Statement();
            ...
            // Finally, place the endSwitch label so that any 'break;' jumps here.
            endSwitch.here();
