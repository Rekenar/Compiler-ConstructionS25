...
public class Parser {
	...
	public static boolean symDump = false;  // new: dump symbol tables?
	public static MJ.SymTab.Obj progObj;   // ← new: remember the program node
	...
	private static void MethodDecl() {
		...
		if (symDump) {
			System.out.println("==< symbol table for method: " + curMethod.name + " >==");
			Tab.dumpScope(curMethod.locals);
		}
	...
	private static void Program() {
		check(program_);
		check(ident);
		progObj = Tab.insert(Obj.Prog, t.val, Tab.noType);
		Tab.openScope();
	...
	public static void parse() {
		...
		if (symDump) {
			System.out.println("==< program scope >==");
			Tab.dumpSingleScope(progObj.locals);
		}
	}
