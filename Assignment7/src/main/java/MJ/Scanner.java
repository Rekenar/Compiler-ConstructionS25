...
public class Scanner {
	...
	// Scans an integer or a ‘normal’ float (no exponent)
// i.e. sequences like 123 or 8.25 but not “.5” or “1e-3”
	private static void readNumber(Token t) {
		...
		// 3) form the token’s text
		t.val = new String(lex, 0, i);

		// 4) parse into the appropriate numeric field
		if (isFloat) {
			t.kind = float_;                // or number, as you prefer
			try {
				t.floatVal = Float.parseFloat(t.val);
			} catch (NumberFormatException e) {
				t.floatVal = 0.0f;
				error("float literal too large");
			}
		} else {
			t.kind = number;
			try {
