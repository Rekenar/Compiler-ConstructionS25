...
public class Parser {
	...
	private static Operand Expr() {
		...
		if (sym == minus) {
			scan();
			x = Term();
			if (x.type != Tab.intType && x.type != Tab.floatType) error("integer or float operand required");
		...
		while (sym == plus || sym == minus) {
			...
			Operand y = Term();
			Code.load(y);
			if ((x.type != Tab.intType || y.type != Tab.intType) && (x.type != Tab.floatType || y.type != Tab.floatType))
				error("integer or float operand required");
			Code.put(op);
		}
		return x;
	}
	...
	private static void Statement() {
		...
		// "read" '(' Designator ')' ';'
			...
			x = Designator();
			check(rpar);
			if (x.type == Tab.intType || x.type == Tab.floatType) Code.put(Code.read);
			else if (x.type == Tab.charType) Code.put(Code.bread);
			else error("can only read int, float or char variables");
		...
		// "print" '(' Expr [',' number] ')' ';'
			...
			Code.load(y);
			if (x.type == Tab.intType || x.type == Tab.floatType) Code.put(Code.print);
	...
	private static Operand Term() {
		...
		while (sym == times || sym == slash || sym == rem) {
			...
			Code.load(y);
			if ((x.type != Tab.intType || y.type != Tab.intType) && (x.type != Tab.floatType || y.type != Tab.floatType))
				error("operands must be of type int or float");
			Code.put(op);
		}
		return x;
	}
